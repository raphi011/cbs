...
What a credit transfer has no equivalent of is the debtor's **refund** — the 8-week, no-questions-asked claim on a settled collection. A payer who simply changes their mind about a credit transfer must ask for a *recall*, which the beneficiary can refuse; that is a \`camt.056\`, and this system does not implement it. Two simplifications inside the return itself: it settles immediately rather than being batched into a later R-cycle, and no return window is enforced — \`ReturnPaymentTx\` asks only whether the scheme allows returns and whether the payment is \`Settled\`. What it *does* ask about the money depends on direction, and on one rule: **a bank can refuse a leg only if it posts it before it sends.** The returning bank posts first, so on a credit transfer the payee's bank can refuse a clawback it cannot fund and no \`pacs.004\` is ever built; on a collection the creditor's bank hears about the return only after the reserves have moved, so it forces the clawback — the payer's 8-week right is unconditional, and the creditor's bank carries the credit risk. That is why creditor banks vet their creditors.`,
  },
  "settlement-delay": {
...
None of this says no lookup for the far side's name exists anywhere in the system — a separate, pre-submission one does: \`GET /directory\` resolves an IBAN across the network and does read the resolved account's name off the payee's own bank's register. What it says is narrower, and about the payment itself: nothing that ends up stored as \`PartyDetails\` is ever the product of one bank reading another's register. That used to be false — \`payment.partyTx\` fetched the far side's name out of the *other* bank's deposit register on the happy path of every submission, a bank reading a book that was never its own — and closing it is exactly what makes building an outbound message today read nothing across the boundary it crosses: the name travels on the instruction because there is nowhere else a payment may take it from.`,
  },
  "audit-trail": {
    title: "Audit trail",
