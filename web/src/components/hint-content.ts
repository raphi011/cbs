...
A payment records the address it was reached by whether or not the caller quoted one: the bank that owns a leg fills in that account's single identifier in the scheme's scheme, and refuses to choose (\`ErrAmbiguousAddress\`) when there are several. The submitting bank does this for its own side; the far side's address is back-filled by the *other* bank when the message reaches it, because until then nobody could look that account up. That is true of the **address** only — the far side's **name** is never looked up by anyone, at any point: see [[counterparty-details]]. The address is a record, not an identity — which is why a [[mandate]] compares its parties by bank and account only. Reissuing the debtor's IBAN would otherwise kill every mandate on the account, permanently.
...
**Uniqueness stops at the bank.** A \`deposit.Register\` spans one book — the correct boundary, not a shortcut: a bank-issued identifier is globally unique by construction (an IBAN carries its bank's code, a PAN its BIN) while a proxy alias like a phone number carries no issuer, which is why proxy lookup needs its own central service and this system has none. \`Register.checkIdentifierFreeTx\` enforces it at write time, within one bank, deliberately with no \`UNIQUE\` constraint behind it, since one only \`store/pg\` could hold would let it disagree with \`store/mem\`. It runs through the same lookup, so it inherits the same comparison rule whole: \`SE89-AURORA-1001\` and \`SE89AURORA1001\` are one address, and the second spelling is refused (\`ErrIdentifierTaken\`). That is the point rather than a side effect — an account holding two spellings of one address resolves either way, so no lookup would ever complain about it. \`payment.Network.ResolveIdentifier\` is what makes that safe **for routing**: it sweeps every member bank at read time and answers \`ErrIdentifierAmbiguous\` rather than guessing, catching both a duplicate the missing constraint let a race create within one bank and a collision across two banks that no single register could ever see. For routing only, though — \`SubmitPaymentTx\` is handed an account id and never resolves, so two accounts sharing an address both stay payable by id. The accounts are distinct and real; what is ambiguous is the address.`,
  ...
  "counterparty-details": {
    ...
    body: `A payment names two parties, and only one side is ever checked against anything. The submitting bank fills in **its own** side from itself — its own BIC, and the account holder's name straight off its own deposit register — because it is the authority on its own customer. The **counterparty's** bank and name are never looked up anywhere; they are asserted on the instruction (\`payment.PartyDetails{Agent, Name}\`, stored on the payment as \`DebtorDetails\`/\`CreditorDetails\`), because a real payer's bank holds no register of another bank's customers to check them against. The receiving bank cannot correct or verify either field — it only checks that both are present, refusing an unnamed counterparty outright (\`ErrCounterpartyNotNamed\`), because there is nowhere in the system it could check them against.

This used to be a lookup rather than an assertion: \`payment.partyTx\` fetched the far side's name out of the *other* bank's [[account-addressing|deposit register]], on the happy path of every submission — a bank reading a book that was never its own. That crossing is closed: building an outbound message today reads nothing across the boundary it crosses, which is what a real payment network does — the name travels on the instruction because there is nowhere else for it to come from.`,
  },
  "audit-trail": {
    title: "Audit trail",
