...
There is no pre-submission lookup either: \`GET /directory\` answers out of one bank's own register, so nothing in this system resolves an address across banks and there is no cross-bank name to reach a payment even in principle. Nothing that ends up stored as \`PartyDetails\` is ever the product of one bank reading another's register — fetching the far side's name out of the *other* bank's deposit register would be a bank reading a book that was never its own, and building an outbound message reads nothing across the boundary it crosses: the name travels on the instruction because there is nowhere else a payment may take it from.`,
  },
  "audit-trail": {
    title: "Audit trail",
