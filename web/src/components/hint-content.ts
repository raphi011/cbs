...
**Across two banks the collision has moved rather than gone.** Minting cannot produce one — an address carries its issuer's [[bank-code]], so two banks holding one address would take two banks allocated one code. Nothing here refuses that yet: this network has no registry of who issued what, so each bank would answer confidently about its own account and neither could see the other. It is a fault in an allocation rather than in a register, which is why no register can catch it. And it is a claim about routing only — a submission is handed an account number and never resolves, so two accounts sharing an address both stay payable by number. The accounts are distinct and real; what is ambiguous is the address.`,
  },
  "address-issuance": {
...
**What is missing here is the table.** Knowing which bank a code belongs to takes a directory of every participant's allocation, and this network has none — which is why a payer is still asked for the payee's bank alongside the address. SEPA is IBAN-only because every bank subscribes to such a table, not because routing is computable from an address. See [[counterparty-details]].`,
  },
