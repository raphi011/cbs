...
None of this says no lookup for the far side's name exists anywhere in the system — [[account-addressing]] covers a separate, pre-submission one, \`GET /directory\`, which resolves an IBAN across the network and does read the resolved account's name off the payee's own bank's register. What it says is narrower, and about the payment itself: nothing that ends up stored as \`PartyDetails\` is ever the product of one bank reading another's register. That used to be false — \`payment.partyTx\` fetched the far side's name out of the *other* bank's deposit register on the happy path of every submission, a bank reading a book that was never its own — and closing it is exactly what makes building an outbound message today read nothing across the boundary it crosses: the name travels on the instruction because there is nowhere else for it to come from.`,
  },
  "audit-trail": {
    title: "Audit trail",
