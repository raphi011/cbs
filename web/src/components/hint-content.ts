...
**A queue outlives the process holding it.** The files waiting for a subscriber are rows in the hosting institution's own database (\`ebics_queue\`), and so is the record of every order that institution has been sent (\`ebics_orders\`). Neither is bookkeeping for its own sake. A file released into a receiving bank's queue is an obligation the [[settlement-finality|reserves have already moved against]], so a host that lost it would lose the instruction with the money final and nothing anywhere refusing anything; and a subscriber told only that its file arrived can learn what became of it from nowhere else.

Storing them raises the obvious objection — a queue is one institution holding bytes addressed to another, and that is only acceptable while those bytes stay **opaque to the holder**. What keeps them opaque is not that they are unstored. It is that the only thing able to reach them is the transport itself, which knows about subscribers, order ids and payloads and has no way to name a payment, an amount or an agent. A clearing house reading a file it is merely carrying is not something to be resisted; it is something there is no way to express.
...
**Each share is discharged on its own**, when the bank it is addressed to has actually been handed it. Putting a file in a bank's [[download-queue|queue]] and writing off the obligation behind it are two separate acts, so a share the clearing house could not hand over stays exactly where it is — still owed, against reserves that have already moved — while its neighbours in the same cut-off go. Writing all of them off together would discharge the one that never left; leaving them all standing would hand some bank its instructions twice, and a bank that applies the same file twice credits the same customer twice.
...
**The window is three days, and it gates nothing.** SEPA gives a receiving bank three *banking business* days to send back a credit it cannot apply, and they are counted on the [[target-calendar|settlement calendar]]: a balance arriving on a Thursday is due back the following Tuesday, and the weekend is no part of the window. The **age** beside it is still calendar days, so a lot four days old and not yet overdue is the weekend showing through — which is why the report carries the due date as well. The window gates no act either way: it is a deadline on the bank's *obligation* and not permission to begin, a return is available for the whole life of a settled payment, and the window only decides which line of the report is printed in bold.
