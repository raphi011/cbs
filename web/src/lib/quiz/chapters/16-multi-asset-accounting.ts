...
export const chapter: Chapter = {
  ...
  questions: [
    ...
    },
    {
      ...
      answers: [0, 1, 2],
      explanation:
        "[[asset|An asset definition is defined in code]], not stored: 'BTC has 8 decimal places' is a fact about the world, true in every book, and storing it per book would only make disagreement representable. It is the same call [[scheme-asset|the payment schemes make]] — a scheme is defined in code, its settlements are rows. An account's asset is fixed at creation and never changes; a customer who wants euro *and* dollars gets a second account, which is why in practice they get a second IBAN. Neither of those is a line of the chart of accounts: each is a position under that asset's customer-deposit [[ledger-vs-subledger|control account]], so the two-currency customer is one obligor named under two different lines. The **class** carries no behaviour at all — it exists so a console can tell a currency from a token without pattern-matching on the code.",
    },
    {
      kind: "mc",
      id: "ch16-q16",
