...
export const chapter: Chapter = {
  ...
  questions: [
    ...
    },
    {
      ...
      answer: false,
      explanation:
        "No bank reads another bank's register to build a payment — that crossing is what closed, and it's what this question asks about. The [[counterparty-details|payee's name]] is asserted on the instruction instead: the payer types it, and the submitting bank stores it in `Payment.CreditorDetails` (`payment.PartyDetails{Agent, Name}`) exactly as received, refusing to submit at all if it is missing (`ErrCounterpartyNotNamed`). The submitting bank fills in only its OWN side from its own register — it is the authority on its own customer, never on the other bank's. (A separate, pre-submission lookup, `GET /directory`, does resolve a name across the network for the payer to see — but that answer is never wired into the payment; what gets stored is what was typed.)",
      explore: { label: "View payments", href: "/clearing-house/payments" },
    },
  ],
};
