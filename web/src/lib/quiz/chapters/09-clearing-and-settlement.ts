...
export const chapter: Chapter = {
  ...
  questions: [
    ...
    },
    {
      ...
      answer: 1,
      explanation:
        "[[settlement-finality|Settlement is final at the central bank]] and the participants catch up afterwards. Once that one unit of work commits, the reserves have moved and nothing a member does next unwinds it. In the EU this is not a modelling convenience but the **Settlement Finality Directive** (98/26/EC), which exists so that one participant's failure cannot reach back into a batch that has already settled. What the failed posting leaves is an [[unreconciled-position|unreconciled position]] — a settlement-advice row still at *Advised*, and a [[clearing-suspense|clearing suspense]] that has not returned to zero. A refusal *before* the commit is the other outcome, and it is equally decisive: `RJCT`/`AM04`, nothing posted anywhere, every payment still Cleared.",
      explore: { label: "View settlements", href: "/clearing-house/settlements" },
    },
    {
      kind: "multi",
