...
export const chapter: Chapter = {
  ...
  questions: [
    ...
    },
    {
      ...
      answer: 1,
      explanation:
        "**A bank exists before it joins a scheme.** [[bank-founding|Founding]] gives it a book, a chart of accounts, its [[participant-assets|per-asset plumbing accounts]] and a deposit product, and its own book is then unrestricted. What it lacks is everything belonging to somebody else: no central bank holds a [[settlement-account|settlement account]] for it and no clearing house holds a [[routing-roster|routing entry]], so nothing it takes part in can settle.\n\n**Taking money in is the trap.** It is tempting to expect funding to be refused with a `422` naming the membership, \"because cash paid in raises the bank's reserve at the central bank in the same [[unit-of-work|unit of work]], and there is no reserve to raise\". That would misdescribe banking: a bank's counter has nothing to do with its central bank account. A founded bank can open its doors and take deposits, and it holds what it takes as [[vault-cash|vault cash]] — its own money, in its own book.\n\nWhat it cannot do is turn that cash into reserves. That is a [[lodgement]], a request to the central bank rather than an entry the bank makes, and it is refused with `422` for a bank that has no reserve account to lodge into. The check moved to the act it was always true about. Paying is refused for the same reason and in the same way, of both parties — a payment to or from such a bank could otherwise be cleared and then stop the whole cut-off, because the clearing house cannot name a non-member in the settlement instruction. The answer that calls Founded a **half-written record** wants admission to be one transaction writing all three institutions' records at once, \"so a bank can never exist without the accounts it needs\" — and no real admission has that guarantee. Both states are ordinary: [[bank-admission|admission is a conversation]], so a bank read straight back may still be `Founded` and perfectly healthy.",
    },
    {
      kind: "multi",
      id: "ch11-q17",
