...
export const chapter: Chapter = {
  ...
  questions: [
    ...
    },
    {
      ...
      prompt:
        "A bank has founded itself — book, chart of accounts, per-asset plumbing, a deposit product — and no scheme has answered it yet. What can it do in the meantime?",
      options: [
        "Nothing worth having. Until the scheme answers, the bank is a half-written record and none of its own book is usable",
        "Publish products, add ledgers and run its own book — but open no customer account, because addressing one needs a bank code the bank has not been allocated yet",
        "Everything a member can do. The only thing missing is a line in a directory, and nothing reads it until a payment is sent",
        "Open customer accounts and take cash in, but not put that cash on reserve, because only the central bank can credit a reserve account",
      ],
      answer: 1,
      explanation:
        "**A bank exists before it joins a scheme.** [[bank-founding|Founding]] gives it a book, a chart of accounts, its [[participant-assets|per-asset plumbing accounts]] and a deposit product to sell. What it lacks is everything belonging to somebody else — and the sharpest of those is an **address range**. Every account is opened with an IBAN minted under the country and [[bank-code|bank code]] its bank was allocated, that code is a registry's to give out, and a bank no registry has answered can open no customer account at all. It applies to a market; it is *given* a code.\n\n**Taking money in is the trap, and it is a trap about the counter rather than the address.** It is tempting to expect funding to be refused \"because cash paid in raises the bank's reserve at the central bank in the same [[unit-of-work|unit of work]], and there is no reserve to raise\". That would misdescribe banking: a bank's counter has nothing to do with its central bank account. Cash over the counter is one institution's act, held as [[vault-cash|vault cash]] in the bank's own book, and nobody else has to agree. Such a bank simply has no customer to take it in *for* — which is why the last answer is wrong about the reason rather than about the refusal.\n\nWhat it cannot do is turn cash into reserves. That is a [[lodgement]], a request to the central bank rather than an entry the bank makes, and it is refused for a bank with no reserve account to lodge into. Paying and being paid are refused too, of both parties — a payment to or from such a bank could otherwise be cleared and then stop the whole cut-off, because the clearing house cannot name a non-member in the settlement instruction. The **half-written record** answer wants admission to be one transaction writing all three institutions' records at once, \"so a bank can never exist without the accounts it needs\", and no real admission has that guarantee. The **only a line in a directory** answer has it backwards: [[bank-admission|the three rows]] are three institutions' separate records, and the two the bank does not own are what it is waiting for.",
    },
    {
      kind: "multi",
      id: "ch11-q17",
