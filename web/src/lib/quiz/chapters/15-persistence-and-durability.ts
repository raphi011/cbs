...
export const chapter: Chapter = {
  ...
  questions: [
    ...
    },
    {
      ...
      answer: 1,
      explanation:
        "One row belonging to three institutions is a shared-store artefact, and the reserve postings were reading straight through it. The three rows are in three **different databases** now (see [[store-split]]), so the old read is not merely discouraged — the settlement agent has no `banks` table to make it against. Splitting it gives each institution a record it could hold **alone**, which is the whole test: `banks` is the bank's own (its status, and the admission it accepted), `settlement_members` is the [[settlement-account|central bank's record of an account it opened]], and `roster_entries` is the [[routing-roster|clearing house's record of where to send a message]]. Each has exactly one writer. The two that are not the bank's are keyed by **BIC** rather than by a bank id, because neither institution allocates or is ever told one — what a message carries is a BIC — so a bank id there would be an identifier its owner had no way to have learnt and no way to check. (The settlement agent writes a fourth row in the same conversation, in `bank_codes`, and that one is keyed by country and code instead: an allocation is national, and a code is unique only within the country that issued it.) The answer about **each institution needing its own copy of the same data** is the opposite of what happened: the rows hold *different* things, and the clearing house's deliberately holds no account identifier at all.",
    },
    {
      kind: "multi",
      id: "ch15-q16",
