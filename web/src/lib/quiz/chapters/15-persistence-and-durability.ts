  ...
  questions: [
    ...
    },
    {
      ...
      options: [
        ...
        "Every file an institution has sent or received",
        "A bank's instructions waiting for that bank's own cut-off",
      ],
      answers: [0, 1, 2, 3, 5, 6, 7],
      explanation:
        "Everything the **domain** stores gets a table: transactions, entries, deposit accounts, [[holds]], [[snapshot|snapshots]] and [[audit-trail|audit events]]. A balance is not stored at all — it is [[derived-balance|computed from the entries]] on demand.\n\nSo does everything an institution **owes and has not yet handed over**. Between a cut-off and its settlement the clearing house is holding each receiving bank's share of every file it took in (`held_files`, with `held_file_transactions` saying which of that file's transactions are whose), and a return it has sent on for settlement waits in `held_returns` until the answer comes back. Those are obligations rather than caches: reserves move against them, so an institution that lost them would leave the money final and a payee's bank never told the payment exists.\n\nSo does a [[download-queue|download queue]] (`ebics_queue`), for exactly the same reason one step later: a share released into a receiving bank's queue is an obligation the reserves have *already* moved against, and an institution that lost it in there would lose the file with nothing anywhere refusing anything.\n\nThe objection to storing a queue is worth stating because it is a real one. A queue is one institution holding bytes addressed to another, which is only acceptable while those bytes are **opaque to the holder** — and a table looks like an invitation to read them. What keeps them opaque is not that they are unstored: it is that the only thing that can reach the table is the transport, whose entire vocabulary is subscribers, order ids and payloads, and which cannot name a payment, an amount or an agent. A clearing house looking inside a file it is merely carrying is not a query somebody might write carelessly; it is a query there is nothing to write with.\n\nAnd so does every file an institution has **sent or received** (`messages`, with `message_payments` naming the payments inside each one). A crossing is an event both endpoints saw, so one file that moves is two rows in two databases and neither is the other's copy. At a host this keeps the same bytes twice for one hop — the transport's copy is opaque and the institution's is parsed and joined to payments — and at a member bank there is nothing to duplicate: EBICS has no push, so a bank hosts nothing, and what it uploaded and what it collected existed nowhere at all once the call returned.\n\nOne thing genuinely is not stored, and it is a defect rather than a boundary: a bank's [[payment-hub|hub]] of instructions waiting for its own cut-off, whose debtor legs are already committed.",
    },
    {
      kind: "mc",
      id: "ch15-q6",
    ...
    },
    {
      ...
      prompt:
        "Admitting a bank writes rows in three institutions' databases — the bank's own, the central bank's record of the account it opened, the clearing house's routing entry — rather than one `participants` row carrying all of it. What forces the split?",
      options: [
      ...
      answer: 1,
      explanation:
        "One row belonging to three institutions is a shared-store artefact, and the reserve postings were reading straight through it. The three rows are in three **different databases** now (see [[store-split]]), so the old read is not merely discouraged — the settlement agent has no `banks` table to make it against. Splitting it gives each institution a record it could hold **alone**, which is the whole test: `banks` is the bank's own (the admission it accepted, and the account numbers it was told), `settlement_members` is the [[settlement-account|central bank's record of an account it opened]], and `roster_entries` is the [[routing-roster|clearing house's record of where to send a message]]. Each has exactly one writer. The two that are not the bank's are keyed by **BIC** rather than by a bank id, because neither institution allocates or is ever told one — what a message carries is a BIC — so a bank id there would be an identifier its owner had no way to have learnt and no way to check. (The settlement agent writes a fourth row in the same unit of work, in `bank_codes`, and that one is keyed by country and code instead: an allocation is national, and a code is unique only within the country that issued it.) The answer about **each institution needing its own copy of the same data** is the opposite of what happened: the rows hold *different* things, and the clearing house's deliberately holds no account identifier at all.",
    },
    {
      kind: "multi",
      id: "ch15-q16",
