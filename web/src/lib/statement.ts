...
export type StatementRow = {
  ...
  description?: string;
  direction: Direction;
  delta: number; // signed minor units: the account's normal-balance direction +amount, the opposite −amount
  runningBalance: number; // minor units after this row, signed by the account's normal balance
  contra: ContraRef;
...
export function buildKnownAccounts(participant?: Participant): Record<string, string> {
  ...
  const roles: [string, string][] = (participant.assets ?? []).flatMap((a) => [
    ...
    // The contra leg of every deposit, and therefore the most frequently
    // rendered of these. Omitting it turns the commonest row in a customer's
    // statement into an opaque account id, and fails no test.
