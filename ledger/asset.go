...
// participant's per-asset plumbing — is a row that names it.
//
// The earlier design had a writable per-book registry. It could not be honoured:
// a participant's suspense, reserve and settlement accounts are provisioned only
// when it joins the network, so an asset registered afterwards produced accounts
// that could never hold money. A list the system can actually deliver on is
// smaller and true.
//
