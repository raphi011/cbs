...
| 5. No-`CHECK` position is a dual-store consequence | **half closed** at Task 17.3 — the dual-store half of the argument is gone with the second store, and the ruling now rests on the reason that never named one: a `CHECK` would put the asset set in two places and make adding one a migration. What is still **open** is the other half of the finding — neither the README nor the schema says that a single-backend production bank would be advised the opposite (deferred balance triggers, revoked `UPDATE`/`DELETE` grants) |
| 6. Trial balance | **open**, and DESIGNED — a branch (`spec/ledger-controls`) carried a plan for it that was retired unbuilt when the world it was written against went away. The shape is worth not re-deriving: `ledger/trialbalance.go`, a walk over `tx.ListAccounts` inside ONE `View` summing each account restated debit-positive — `TrialBalance{AsOf, Rows}`, `TrialBalanceRow{Asset, Debits, Credits, InFlight}`, `Balanced() bool`, and `Book.TrialBalance`/`TrialBalanceTx`. Per book AND per asset, because a single-asset total in a multi-asset ledger balances by accident. No store method and no schema change | — |
...
| 9. Reject future booking dates | **open**, and DESIGNED on the same retired branch — a refusal inside `PostTransactionTx` (`ledger.ErrFutureBookingDate`) plus an exported `Book.CheckBookingDate` that the two end-of-day batches (`deposit.RunEndOfDayTx`, `lending.RunEndOfDayTx`) call ONCE before their loops rather than per posting. The plan's own task order is the part worth keeping: advance every fixture that books into the future BEFORE the rule exists, so the fixture churn and the rule land as separate, reviewable changes | — |
...
| 13. Inbound suspense + return | **half-closed** — the *destination* exists (`Unclaimed Balances (<asset>)`, one per bank per asset) and `SettleAtBankTx` diverts to it; what resolves the balance afterwards does not. See §4.5 | `payment/`, `participant_assets.unclaimed` |
...
- **§4.5's creditor-leg finding is not a one-line fix, and the reason is
  ...
  *Closed by sub-project 8's Task 15 (2026-08-02), and the first sentence of the
  ...
  `SettleAtBankTx` calls `CheckCreditTx` and diverts to it. What resolves the
