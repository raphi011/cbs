...
Product terms in the repo are mutable columns on the instance: `Account.Rate`, `UnarrangedRate`, `DayCount`, `OverdraftLimit`; `Facility.Rate`, `Method`, `MinPayment`. `SetOverdraftTermsTx` (`deposit/register.go:270-273`) **overwrites the rate in place**.
...
The book's full machinery — a `product`/`product_version` table with a content hash, non-overlapping effective intervals enforced by an exclusion constraint, per-parameter-group **pinned vs floating** binding (§24.1.3), overlays (§24.1.4), and a resolution log — is far beyond this repo's scope. The *effective-dated terms record* is not.
...
- **The creditor leg bypasses the deposit layer entirely.** `SettleCycleTx` (`payment/system.go:790-802`) resolves `creditor.glAccountTx(...)` and posts straight into the GL account. A payee whose account was frozen or closed between initiation and settlement is credited anyway. §12.2.3 prescribes the alternative: an unapplicable settled credit books to **inbound suspense**, a return is initiated within the rulebook window (three banking business days for SCT), and the suspense clears via the outbound return posting — "inbound suspense older than the return window is a reconciliation break by definition." The repo has the suspense account already; it lacks the branch.
...
4. README *Next Work*: account-status enforcement on the debit path is already done. The live gap is the creditor leg at settlement.
...
13. **Inbound suspense + return** when the creditor leg cannot be applied at settlement.
