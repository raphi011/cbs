...
**Next in build order.** Each entity gets its own listener bound to its own
identity: one per member bank, one for the central bank, one for the clearing
...
### 6b. Role-scoped web UI — `spec`

Spec: [`superpowers/specs/2026-07-31-role-scoped-web-ui-design.md`](superpowers/specs/2026-07-31-role-scoped-web-ui-design.md)
Plan: [`superpowers/plans/2026-07-31-role-scoped-web-ui.md`](superpowers/plans/2026-07-31-role-scoped-web-ui.md) — written before 6a, partly invalid, to be re-planned
...
`pacs.004` / `pacs.009` over channels, wrapped in a `head.001` business
application header.

Three sub-projects, each with its own spec, plan and branch. The first two are
specified:
...
- **7b, the mesh and the actors** — one goroutine per entity over unbounded
  in-process queues carrying marshalled bytes, `Participant.BIC` and routing by
  `BICFI`, `InitiatePaymentTx` splitting into a debtor bank's half and a
  creditor bank's half with the other half coming back as a `pacs.002`, and
  `api/` becoming genuinely asynchronous behind the `202 Accepted` plus status
  query 6a already shaped the client around. Settlement stays one atomic
  `Store.Update` at the central bank, now **instructed** by a `pacs.009` the
  clearing house sends when it closes a cycle — the fifth message, and the one
  7a did not ship.
