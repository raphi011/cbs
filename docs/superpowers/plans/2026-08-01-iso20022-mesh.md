...
| `web/src/app/central-bank/page.tsx`, `web/src/lib/api/{hooks,endpoints}.ts` | The settle console, which calls the deleted `POST /settlements` through a string no compiler checks — see Task 12 Step 4 |
...
- [ ] **Step 4: Delete `POST /settlements`.** Remove the route from `centralBankRouter()` and the handler; keep the reads. Update `api/server_test.go`'s calls to it.
  ...
  **This endpoint has a live consumer that no compiler will find.** 6b built the
  central bank's settle console on it: `web/src/app/central-bank/page.tsx` →
  `useSettleCycle` (`web/src/lib/api/hooks.ts`) → `api.settleCycle`
  (`web/src/lib/api/endpoints.ts`), whose path is `cb("/settlements")` — a
  string, so `tsc`, `eslint` and `next build` all stay green and the screen 404s
  at runtime. Deleting the route is a **web change too**, and it needs a decision
  rather than a deletion: after this task the instruction is emitted by closing a
  cycle, so a human no longer settles one. Either drop the settle action and let
  the console read the settlements it now performs automatically (the four reads
  6b added stay, which is what "keep the reads" is for), or keep a manual
  override and give it a route to call. Whichever, `/central-bank`'s settlement
  section and `useSettleCycle` change in this commit — and the `RJCT`/`AM04`
  answer this task introduces is a state that console should be able to show,
  which today it cannot.
- [ ] **Step 5: Run** both stores. Expected: PASS. Then the web gate from `web/`:
  `npm run typecheck && npm run lint && npm run test && npm run build`, and
  **load `/central-bank`** — the gate cannot see a dead endpoint behind a string.
- [ ] **Step 6: Commit.**
