...
- **A refund into a closed payer's account still strands.** The settlement half of this gap is closed: `PostCreditorLegTx` — the payee's bank's own act, extracted out of `SettleCycleTx` — calls `CheckCreditTx` before it releases a settled payment out of suspense, and a payee whose account was **closed** between their bank's answer and the cut-off is credited to that bank's **Unclaimed Balances (\<asset\>)** instead. The payment still reaches `Settled`, because it did: the reserves moved and the payee's bank has been paid. Whether the **customer** has been paid is left open, and it is between the bank and its customer. What made the check affordable was not the check — it was having somewhere for the money to go, plus the split that lets one payment at one bank fail without taking the cut-off down with it. (A *frozen* payee being credited is still deliberate, not a gap: the freeze here is a debit block.) What remains is the mirror case on the **return** path: `ReturnPaymentTx` credits the payer's GL account with no such check, so a payer who closes their account after a payment settles and then has it returned is credited into a `Closed` account. The account that fixes it now exists at every bank; using it there is a change to the return path that has not been made.
