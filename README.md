...
**"Never writes" is the claim, and it is narrower than "never reaches".** A submitting bank does read a book that is not its own, on the happy path, every time: the `pacs.008` it sends has to name the payee, and `payment.partyTx` gets that name out of the *payee's bank's* register. The same crossing runs the other way for a collection. It is a shared-store artefact and it is measured rather than assumed — `mesh.TestWhichBooksEachBankActuallyReaches` asserts the exact set of books each actor touches, and the crossing was found by that measurement rather than predicted. What closes it is a domain change and not a transport one: a real payer's bank knows the payee's name because the payer typed it in, so the request would have to carry the counterparty's name. That is [sub-project 8's](docs/expansion-roadmap.md) work; what this system owes it is the number.
...
- **Rejected** is reachable from `Initiated` *and* from `Accepted`, and it is **two halves in two units of work**. The clearing house marks the payment `Rejected` and drops it from any cycle; the *payer's bank* then reverses the debtor leg, on receiving the `pacs.002`. Between the two the rejection has **half-happened**: the payment reads `Rejected` and the customer's money is still in suspense. That is the first operation in this system that can be caught in the middle, and it is deliberately not hidden — a reversal that fails has nobody to answer, so it surfaces as a mesh dead letter rather than as a quiet lie to the payer. Closing it for real needs a way to carry an unreconciled position, which this system does not have.
  A rejected **collection** is told to *two* banks, and only a collection ever is: the bank waiting for the answer is the payee's, and the bank holding the money is the payer's. "One decision, one message" is not a property of this system, because those are two different institutions.
...
- **Five ISO 20022 messages, and what is missing is named rather than implied.** The interbank messages are real: `pacs.008` (credit transfer), `pacs.003` (collection), `pacs.002` (status), `pacs.004` (return) and `pacs.009` (the settlement instruction), each wrapped in a `head.001` business application header, marshalled to XML and *parsed on arrival* — which is what makes `FF01`, the rejection a receiver sends when it cannot read what it was given, a reachable failure mode rather than a decoration. Nothing but bytes crosses between the institutions. What is absent: **`pain.001`/`pain.008`** customer initiation (a customer's instruction arrives over this system's own REST API instead), the whole **`camt`** reporting family — including the `camt.054` a bank would learn of settlement from, and the `admi.002` a real receiver would answer an unreadable file with — **`camt.056`/`pacs.007`** recalls and reversals, **runtime XSD validation** (the golden files *are* checked against the real schemas, but only by `make test-schemas`, which needs `xmllint` and the XSDs under `iso20022/testdata/xsd`; a plain `go test` skips that check, and `ISO20022_REQUIRE_SCHEMAS=1` is what turns the skip into a failure), and **message signing**. There is also no **batching** of customer payments: a `pacs.008` or `pacs.003` this system builds carries exactly one transaction, and one arriving with several is refused — which is why `pacs.002`'s `PART` group status, built in the codec, is never produced. The settlement `pacs.009` is not an exception to that so much as a different thing: it carries one leg per member bank, because netting produces exactly one position per member.
