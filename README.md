...
4. **`payment` — the interbank payment network.** Each participant bank gets its own `ledger.Book` plus a `deposit.Register` and a `lending.Portfolio` over it; funds and status checks for a payment run through that deposit layer, while the multi-leg GL postings (debtor leg, creditor leg, reserve movements) live in the payment layer. Its top-level types are one per institution — `payment.BankNetwork`, `payment.ClearingHouseNetwork` and `payment.CentralBankNetwork` — each embedding a shared core, so an act that belongs to one of them cannot be named through another's handle.
...
**"Never writes" is the claim, and it used to be narrower than "never reaches".** A submitting bank used to read a book that was not its own, on the happy path of every submission: the `pacs.008` it sends has to name the payee, and `payment.partyTx` read that name out of the *payee's bank's* register. The same crossing ran the other way for a collection. It was a shared-store artefact, and it was measured rather than assumed — `cmd/server`'s `TestWhichBooksEachBankActuallyReaches` asserts the exact set of books each actor touches, and the crossing was found by that measurement rather than predicted.
...
**What still makes a misdirected message safe** is unchanged and is what always did the real work: a receiving bank resolves an address in **its own register alone**. A message that reaches the wrong institution has nothing to resolve, so that bank sends the money back with `AC01` — a `pacs.004` and not a rejection, because it is handed the instruction only after the cycle carrying it has settled. `cmd/server`'s `TestCreditTransferToAnUnknownAccountComesBackAsAnAC01Return` is the pin — an address under the right bank's code that the bank does not hold, which is the shape that is still reachable now that a payer cannot name a bank at all.
...
| Clearing Suspense | Liability | In-transit funds that have left a customer but not yet settled between banks. Returns to zero once this bank has booked **both** its halves of the cut-off — the mirror leg from the central bank's `camt.053` and its creditor legs from the instructions the clearing house releases — which is *after* settlement, not at it. |
...
**A third act moves vault cash, and without it a bank with no depositors could hold no money at all.** Cash paid in over the counter is the customers' — the bank holds it and owes it back — so a bank on its first day, before a single account has been opened, would have an empty vault and nothing to lodge. What funds it is a **capital subscription**: its owners paying money in.
...
Same debit as a deposit, and the credit is the whole difference. A depositor is owed their balance and a shareholder is not, which is why one is a liability and the other is [equity](#equity--the-owners-residual-interest) — the one credit on a bank's chart that is not somebody's claim. It runs at **provisioning**, so every bank in this system is capitalised before it has a customer, and it is the only act that puts money there. Its absence had a visible consequence: a prefunded bank with zero customers was not a state this system could reach, so a base state had to stage one nominal depositor per bank and call it funding.

Three acts, then, and each moves the vault a different way: a subscription puts money in, a deposit puts a customer's money in, a lodgement takes money out onto reserve. Nothing else posts to `Share Capital` — there are no retained earnings and no distributions here, so a bank's equity rises exactly once in its life.
...
- **Which bank submits.** A push is submitted by the payer's bank, a pull by the payee's — because a collection is the payee asking for what it is owed. The bank's `POST /payments` refuses a submission from the wrong end (`payment.ErrNotTheSubmittingAgent`, `422`), and it is the *bank's own* refusal rather than a router's: a console that wants to submit as another bank posts to that bank's port, which it can already reach. Half the schemes turn on this, so it is not a corner case — a direct debit posted to the payer's console is one bank's customer trying to make another bank send.
...
**It is caught eventually, and that is the argument for `ErrAssetMismatch`, not against it.** After the cut-off has settled and the instruction has been released to it, the **payee's own bank** comes to post the creditor leg and builds it from *its own* suspense account in the scheme's asset — `creditor.AccountsFor(scheme.Asset())`, in `SettleAtBankTx`, which is that bank's act and not the settlement agent's — so what actually gets posted is:

```
Bank B (creditor leg, on the released instruction):
...
- **Initiated → Accepted** is the **clearing house's** act and nobody else's. It validates the file it was handed and takes each transaction into the open cycle for its scheme, and only then is it `Accepted`. **The payee's bank has not seen it**, and will not until the cycle settles. If no cycle is open the payment is refused — `ErrCycleNotOpen`, which on the wire is a `pacs.002` carrying **`TM01`, "invalid cut-off time"**. That refusal belongs to the clearing house because the cut-off does: a bank that turned its own customer's instruction away because the CSM had no window open would be answering a question it was never asked. It is not a `4xx` on the customer's request, because by the time it is decided that request has long been answered.
...
- **Cleared → Settled** takes **three institutions, of which two post — in two units of work**. The central bank moves the reserves, because the clearing house *asked* — closing a cycle sends a `pacs.009` — and a net payer who cannot cover is answered `RJCT`/`AM04`, after which nothing is posted at all: the cycle stays `Closed` with no settlement against it and every payment stays exactly where the cut-off left it. That settlement is **final** either way, and it is not what marks a payment `Settled`. Only once it is final does the clearing house **release** the file to the payee's bank, and *that* bank writes its own copy and posts the **creditor leg** out of its own clearing suspense into its own customer's account. So a payment is `Cleared` with the reserves already moved for as long as its bank has not acted, and that interval is the **unreconciled position** the message layer makes visible rather than hides.
  **Settle, then release, is the whole ordering** and reversing it invents settlement risk: a receiving bank gets the instruction only once the funds behind it are final, so it never credits a customer against a batch that might still fail. What it costs is that bank's ability to say *no* — see `Rejected` below.
- **Rejected** is reachable from `Initiated` *and* from `Accepted`, and it is **two halves in two units of work**. The clearing house marks the payment `Rejected` and drops it from any cycle; the *submitting bank* then reverses the debtor leg, on receiving the `pacs.002`. Between the two the rejection has **half-happened**: the payment reads `Rejected` and the customer's money is still in suspense. That is the first operation in this system that can be caught in the middle, and it is deliberately not hidden — a reversal that fails has nobody to answer, so it surfaces as a line in the day's report rather than as a quiet lie to the payer. Closing it for real needs a way to carry an unreconciled position, which this system does not have.
  **The clearing house is the only institution that rejects anything**, and one message is all a rejection ever needs, because before finality the submitting bank is the only bank that has heard of the payment. Its refusals are the ones it can make from its own rows: a destination it holds no download queue for (`RC01`), a member its roster does not admit for the scheme's asset, and `TM01`.
  **A receiving bank cannot reject at all.** It is handed the instruction with the money already in its clearing suspense, so what would have been `AC01` for an address it does not hold, or `AM04` for a payer it cannot collect from, is a `pacs.004` **return**. That is what those codes are in SEPA — the rule book's own division is *before settlement, reject; after settlement, return* — and settling before releasing is what puts every receiving bank's objection on the second side of that line.
...
**3. Settlement** — **four** ledger transactions in **four** units of work, belonging to **three** institutions: the central bank posts one, each bank posts its own mirror leg, and the payee's bank posts the creditor leg on top. Only the first is settlement; the rest are the members catching up. The order below is the order the messages arrive in, and for Bank B that ordering is load-bearing rather than presentational — the `camt.053` reaches it before the released `pacs.008`, which is what puts money in the suspense the creditor leg then draws on.
...
Bank B, on the pacs.008 the clearing house releases once the cycle is final:
               Debit  Clearing Suspense 3000             // creditor leg: release...
               Credit Bob (Liability)   3000             // ...funds to Bob
```

Bank B sees the instruction for the first time in that last step. That is **settle, then release**: the file carrying the payment is held at the clearing house until the reserves behind it have moved, so no bank ever credits a customer against a cut-off that might still fail.
...
**The window is three days, and it gates nothing.** SEPA gives a receiving bank that cannot apply a credit **three banking business days** to send it back (`payment.ReturnWindowDays`), and business days is what it is counted in: the lot's due date is `calendar.AddSettlementDays` from the day the credit landed, so a balance arriving on a Thursday is due back the following Tuesday and the weekend is not part of the window. The lot's *age* is still calendar days, so a lot four days old and not yet overdue is the weekend showing through — which is why the report carries the due date beside the age. The window gates no act either way: it is a deadline on the bank's **obligation** and not permission to begin. A return is available for the whole life of a settled payment, and the window decides only which line of the report is printed in bold.
...
Every layer reaches state through a **store interface** — `ledger.Store`, `deposit.Store`, and one per institution at the payment layer — never through a map it owns. There is one implementation:
...
The interfaces are still interfaces with one implementation behind them, and that is the point rather than an accident: every rule the domain layers state is stated against a store and a transaction interface, and none of them names a table. What the boundary buys is not swappability. It is that you can read `ledger.Book` and know that nothing it decides was decided by a database.

**At the payment layer there is one of each per institution**, and that is the store split made into types. `payment.BankTx`, `payment.CsmTx` and `payment.CentralBankTx` carry 74, 30 and 46 methods, and each carries only what its institution's schema creates: a clearing house's transaction cannot name a deposit account, a bank's cannot name a clearing cycle, and the settlement agent's cannot name a payment at all. There is no interface spanning the three, and the three ways to open a database — `sqlite.OpenBank`, `OpenClearingHouse`, `OpenCentralBank` — each return the type whose methods that schema can answer.
...
What survives the loss is the way the cases were written. `storetest` talks only to the store and transaction interfaces — never to `ledger.Book`, `deposit.Register` or `payment.Network` — and names no table and no dialect, so what each case pins is the **contract**: identity allocation, ordering, idempotency, the rows a scan yields, the audit log, rollback. What a balance IS is not in there — that is a fold in `ledger`, tested against entries with no store at all. That is a suite one implementation can be held to. It is also already organised by institution: the five capability suites take a **bank's** store, because a bank's schema is the only one holding every table they reach, and the clearing house and the settlement agent have suites of their own over the tables their schemas do hold. Since the store split each suite takes its own institution's store type, so a case put in the wrong suite does not compile.
...
**Every absence in that paragraph is also a missing method.** The transaction type each institution's store hands out names only the tables its schema creates, so asking the settlement agent for a payment or the clearing house for a ledger balance is a compile error rather than a refusal at run time. The two column differences are the exception, and they are the one thing no type can state: a bank's `payments` row and the clearing house's are the same table with different columns, so the implementation still chooses the column list.
...
What **two** files have and the third does not is the file transport: `ebics_queue` and `ebics_orders`, in the clearing house's schema and the settlement agent's. Those are the two institutions that are *dialled*; EBICS has no push, so a member bank dials out and hosts nothing, and the files it is owed wait in somebody else's database until it comes to collect them. A queue row is one institution holding bytes addressed to another, and what keeps those bytes opaque is not that they are unstored — it is that nothing an institution can call reaches the table. A bank's store has no `EBICS` method at all.

What **all three** files have, and what closes the largest hole a member bank had, is the **message log**: `messages` and `message_payments`, one row per envelope that institution sent or received, with the file as it travelled and the payments it carried. A crossing is an event both endpoints observed, so one file that moves is two rows in two databases, and neither is the other's copy — there is no cross-institution view here for the same reason there is none of the audit log. At the two hosts it duplicates `ebics_orders.payload` for one hop, and that is accepted rather than overlooked: the transport's copy is opaque bytes with an order status, the institution's is parsed and joined to payments, and the transport must not read a domain table any more than the domain may depend on the transport's. At a member bank there is nothing to duplicate. EBICS has no push, so a bank hosts nothing and held no record of any file at all: what it uploaded and what it collected existed nowhere once the call returned.

What the log deliberately does *not* carry is a **phase**. Which step of a business day a file moved in is the *deployment's* declared sequence, and an institution does not know a deployment exists — a phase column would be an institution writing down something it cannot know.
...
A partial success there would leave money that had left one bank without arriving at another, and no retry could tell which half had happened. This is why one concrete transaction type implements all three layers' transaction interfaces — `payment.BankTx` embeds `deposit.Tx`, which embeds `ledger.Tx` — rather than each layer owning its own.
...
`-database` (or `DATABASE_URL`) is a **file path**. Empty means an in-memory database of its own, which is the default and needs nothing at all; a path means an ordinary SQLite file under WAL. Each institution's constructor applies the embedded migrations either way, so a fresh file is usable immediately. Building the base state is **idempotent** — `seed.Populate` writes it against an empty system and returns without touching a populated one — which is what makes a restart against a file a no-op rather than a second copy of every bank. `POST /admin/reset` clears the store and rebuilds the base state. A **scenario** is not idempotent and is not meant to be: it is a script of operator acts, so running one twice opens two sets of customers.
...
Reserves move against every one of those, which is what makes them obligations rather than caches. `cmd/server`'s two restart tests are the only ones in the repository that can fail for this reason: one drops the process between a cut-off and its settlement, the other between a settlement and the banks collecting what it released, and both ask whether the receiving banks were still handed their instructions. The held files answer the first and the durable queue the second.
...
**One binary, but not one package.** Each institution's code lives in a package that plays exactly one: `node/bank`, `node/csm`, `node/centralbank`. Reading what a clearing house does is reading one of them, and none of the three can name the deployment that built it — each is handed a small environment at construction (a clock, somewhere to log, somewhere to report, and the two addresses it dials) and knows nothing else about the process it shares. `cmd/server` is what is left: the business day in the order it runs, the journal, the reset, the seed — the *simulator*, which is allowed to see every institution at once precisely because it is not one of them. That is why an operator's buttons (advance the day, reset, list the members, submit as whichever bank the scheme says) are served on an institution's listener but declared on a separate `Operator` interface: they are the deployment's acts, and the type system now says so. What it buys is that reading one institution is reading one package — **not** a step towards three processes, which are not wanted here and are ruled out in the same record.

Nor does it answer on one shared payment network. A network is **one institution's** handle, and it is one institution's TYPE: `BankNetwork`, `ClearingHouseNetwork` or `CentralBankNetwork`, each built with an `Identity` saying which entity it is, and each listener holds the one belonging to its own — the bank whose port it is, or the clearing house, or the central bank. A bank's handle reaches 68 of the payment layer's acts, the clearing house's 42 and the settlement agent's 30; asking one for another's act is a compile error rather than a refusal. That is what lets an act ask "whose book?" and get an answer without being told; before it, every act about a particular bank took that bank as an argument, so the caller asserted who it was and the domain believed it. `payment.Networks` is the factory the process holds, and it is the only thing in the repository that holds more than one institution's view.
...
| `GET /scenarios`, `POST /scenarios/{id}` | what an operator may trigger, and one of them |
| `POST /admin/reset` | clear every store and rebuild the base state |
...
**A phase is a door, so a day can be run as far as one and stopped.** `POST /clock/day` runs what is left of the day and answers with what all of it moved together; `POST /clock/phases/clearing` runs the day *through* the clearing — the phases still outstanding before it, and then it — and stops. That is how a reader watches a file *wait*: after the cut-off the `pacs.008` is sitting in the clearing house's order log, and after the clearing the `pacs.002` is sitting in the payer's bank's download queue with nobody having come for it. Each door answers with the same three lists a day's report carries, narrowed to what its run did, and names the phases that run took — and **the clock does not move**, because a phase is a step inside a day and only advancing the day ends one.

A phase is **named and never parameterised**: the key is the phase's own name spelt for a URL, `GET /clock/phases` is where the names come from, and a name the day does not declare is a `400`. A door takes one name and nothing else — no range, no list, no phase narrowed to one queue — and the sequence it runs is the DAY's, which is why naming a later phase is not composing a run: it is the same prefix `POST /clock/day` would have run, stopped earlier and without the roll. A phase the day has already run has nothing outstanding before it, so naming it runs it alone, and asking twice runs it twice. `settlementOnly` is in the listing rather than enforced by the door: on a day the scheme is shut `POST /clock/day` skips the clearing phases, and an operator who names one has already decided they want it.

**The day records how far it has got, so advancing FINISHES it rather than starting it again.** `business-date` is one line holding the instant and the phase last completed, written together so a process killed between them cannot leave them disagreeing. The marker moves phase by phase as each one runs in turn, and never jumps: that is what lets a door reach FORWARD safely, because naming a later phase runs the ones before it rather than skipping to it — and settling before releasing is the one order the whole system is built to keep. A phase the day is not holding outstanding — one already behind the marker, one the scheme's closure means today does not run — runs **out of turn**: it runs, it marks nothing, and the day runs it in its place. `GET /clock/phases` carries `completed` per phase, `POST /clock/day` answers with the phases *this call* ran, and moving the date clears the marker in the same write. A phase can still be run twice by asking twice; that a second run is a no-op is still an accident of each phase's own design — a cut-off empties the hub it drains, the clearing house works through a pending list, the end-of-day advancement guards make a second close a no-op — rather than a rule with a test behind it.

**A fresh deployment holds a BASE STATE, and a scenario is what fills it.** Boot leaves four banks founded, admitted at the settlement agent, on the clearing house's roster, subscribed to the routing directory, priced, capitalised and placed on reserve, each with one depositor who has paid €3,000 in over the counter — and nothing else. There is no second customer, no payment and no facility until somebody triggers one of `GET /scenarios`. That one depositor apiece is what makes the base state something a reader can pay out of by hand: an address to send to, money behind it, and a bank on both ends that can settle. Five ship: a payment carried the whole way, a payment handed over after the window shut, a settled payment returned, a borrower who falls behind, and the *demo network* that fills all four banks at once.

**Every scenario drives the doors above, and none of them writes a row behind one.** That is the rule the arrangement exists to establish, and it is not stylistic: a payment placed directly into the database reads as *Settled* on every screen and appears in **no message log and on no graph**, because no file was ever built — and the first thing anyone does with such a payment is the one thing it cannot support. So a scenario submits through a bank, reaches that bank's cut-off, and lets the clearing house and the settlement agent do their own halves; its report carries the same three lists a day's does, and a test asserts each one moved at least one file and left `payment/recon` clean. The refusal above is real for the same reason: nothing in that scenario names a reason code, and the `TM01` its payment comes back with is the clearing house's own judgement about a window it had already closed.

Three costs are accepted rather than engineered away, and the UI states each before the click. **A scenario advances the shared business date**, often by months — the demo network by about eight — so every open tab moves with it. **Two cannot run side by side**, because there is one clock and one set of databases behind every port, and a scenario takes the lock `POST /clock/day` and `POST /admin/reset` take. **There is no undo**: a reset is the only way back, exactly as it is for the day.
...
> Without `DATABASE_URL` the server runs on an **ephemeral in-memory database**: all state resets on restart, and `POST /admin/reset` rewinds to the base state at any time. With one, it is a SQLite file and the data outlives the process (see [Persistence](#persistence)). Either way it is a learning and prototyping tool, not a production service.
