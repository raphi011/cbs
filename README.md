...
**A phase is a door, so a day can be run a step at a time.** `POST /clock/day` runs what is left of the day and answers with what all of it moved together; `POST /clock/phases/clearing` runs one and stops. That is how a reader watches a file *wait*: after the cut-off the `pacs.008` is sitting in the clearing house's order log, and after the clearing the `pacs.002` is sitting in the payer's bank's download queue with nobody having come for it. Each door answers with the same three lists a day's report carries, narrowed to what that phase did — and **the clock does not move**, because a phase is a step inside a day and only advancing the day ends one.
...
**The day records how far it has got, so advancing FINISHES it rather than starting it again.** `business-date` is one line holding the instant and the phase last completed, written together so a process killed between them cannot leave them disagreeing. A door records its phase only when that phase was the one due next: a phase run **out of turn** runs, marks nothing, and is run again in its place, because a marker that jumped to it would let the day skip the phases before it — and settling before releasing is the one order the whole system is built to keep. `GET /clock/phases` carries `completed` per phase, `POST /clock/day` answers with the phases *this call* ran, and moving the date clears the marker in the same write. A phase can still be run twice by asking twice; that a second run is a no-op is still an accident of each phase's own design — a cut-off empties the hub it drains, the clearing house works through a pending list, the end-of-day advancement guards make a second close a no-op — rather than a rule with a test behind it.
