...
What closed it was a domain change, not a transport one, and it is the fact a real payment network runs on: **a payer's bank knows the payee's name because the payer typed it onto the instruction, not because building the instruction can look it up** — banks hold no register of each other's customers for a payment to consult. (This parenthesis used to grant an exception to that sentence: `GET /directory` could resolve a payee's name *across the network* before any instruction existed, and it was what let a send form show which bank an IBAN belonged to. The exception is gone. That lookup answers out of one bank's own register now, so it can tell a customer about their own bank's accounts and nothing about anybody else's — and the send form asks the payer for the payee's BIC instead of looking it up. The sentence above holds without qualification.) `InitiatePaymentRequest` now carries the counterparty's **name** alongside the account; `Payment.DebtorDetails`/`CreditorDetails` (`payment.PartyDetails{Agent, Name}`) store what each side asserts, filled from the submitting bank's own register for its own side and taken verbatim from the request for the other; and `SubmitPaymentTx` refuses an unnamed counterparty outright (`ErrCounterpartyNotNamed`) rather than filling the gap by reading someone else's book. `payment.partyTx` no longer exists — building an outbound message reads nothing across the boundary it crosses.

**The name, and the BIC beside it — and this paragraph says the opposite of what it used to.** The other half of `PartyDetails` is the counterparty's **agent**, its bank's BIC. It was *derived*: `SubmitPaymentTx` read the **bank row** for the participant the payment already named and ignored whatever the caller supplied. That was Task 14's fix for a real routing hole, and the hole was measured — the agent goes out as `CdtrAgt`/`DbtrAgt` and the clearing house **routes on it** with no store read of its own, so a payer allowed to type it was a payer allowed to choose which bank received the payment: a push whose creditor agent named the payer's own bank came straight back to its sender, and a pull whose debtor agent named the collector had the *collecting* bank post the debit in the payer's bank's book.

**Task 18a reversed it, because the derivation had nowhere left to read from.** The row it read is the counterparty's, and a bank holds only its own. There is no second source: the roster is keyed by the BIC being derived and belongs to the clearing house rather than to a bank, and this network has no IBAN-to-BIC directory service — which is the thing a real originating bank actually derives from. SEPA is IBAN-only because every bank subscribes to such a table, not because routing is computable from an address. So an address here is an IBAN **and** a BIC, which is what SEPA was before February 2016 and what a cross-border transfer still is.

**What makes asserting it safe is the other half of the same commit**, and it is the crossing that used to be recorded here as still open: the receiving bank resolved an inbound address by sweeping **every** member's register (`ResolveIdentifierTx`), so a misdirected message reached a bank that could resolve a payee it does not hold — which is the mechanism underneath *both* measured failures above. That resolution now answers out of the resolving bank's own register alone. A wrongly addressed message has nothing to resolve, the bank that receives it answers `AC01`, and the payer's debit is reversed by the rejection. `mesh.TestAWrongCounterpartyAgentIsRefusedByTheBankItNames` is the pin, and it is the old test reversed. **The payer supplies the name and the bank; naming the wrong bank is refused by the bank it names.**
...
So "resolve this IBAN" is now a question with a bank attached, and the honest answer to "which bank holds this IBAN?" is that nothing in this system answers it — a payer is told the payee's BIC the way they are told the payee's IBAN. It still refuses ambiguity rather than taking the first hit, but the ambiguity it can now see is narrower: two of *this bank's own* accounts claiming one address. A collision across two banks is no longer observable by anyone, because no reader spans both registers. The write-time half of this is a separate check, inside the bank: `deposit.Register.checkIdentifierFreeTx` refuses a second account at the *same* bank taking an identifier another one there already holds (`ErrIdentifierTaken`), deliberately with no `UNIQUE` constraint behind it — a constraint would fire on the race this read-then-write leaves open, which the register does not, and would fire as a constraint violation rather than as `ErrIdentifierTaken`. It runs through `ListDepositAccountsByIdentifier`, so it inherits that lookup's comparison rule *whole*: two identifiers are the same address when their schemes are equal and their values are equal after compaction, which for an IBAN means `SE89-AURORA-1001` and `SE89AURORA1001` are one address and the second spelling is refused. That is the point rather than a side effect — an account holding two spellings of one address resolves either way, so no lookup would ever complain about it. `ResolveIdentifier` is what makes that safe **for routing**: the missing constraint lets a race create a within-bank duplicate, and looking that address up — through `GET /directory`, or anywhere a payment starts from an address — refuses rather than guessing. The comparison to a duplicate *across* two banks used to be drawn here and no longer holds: nothing resolves across banks, so a cross-bank collision is not something this refusal protects anyone from — it is simply invisible, and two customers at two banks may hold one address with nothing in the system able to notice. It is a claim about the *directory*, and only about it: `SubmitPaymentTx` is given an account id and never resolves anything, so two accounts colliding on one address both stay payable by id. That is the right answer rather than a gap — the accounts are real and distinct, and it is the address that is ambiguous, not either of them — but it is why the refusal belongs to the lookup and not to the payment.

**An address is compared in its canonical form, not literally — and for an IBAN those differ.** An IBAN is transmitted and canonically stored without separators and *displayed* in groups; this system stores the readable form (`SE89-AURORA-1001`) because a mod-97 check digit was refused for the same reason, while an IBAN inside a `pacs.008` carries none (`SE89AURORA1001`). They are one address, and compaction does not run backwards, so the only way to match them is to strip the separators from **both** sides: `deposit.Identifier.MatchValue`, which `ListDepositAccountsByIdentifier` uses and which `storetest` pins, because the store re-implements that Go function in SQL and the compiler cannot check the two against each other. Without it a bank emits an address it then cannot resolve — every seeded account unreachable from a message it produced itself. The rule is the IBAN scheme's alone: nothing else here has a display form, and stripping punctuation out of a card PAN would merge two addresses a scheme keeps apart. What is stored and what a payment records are untouched; only the comparison is canonical.
...
What a founded bank can do is its own book, and that part is unrestricted: it opens customer accounts, publishes products, adds ledgers. **It can also take money in**, and this paragraph used to say the opposite — that it cannot fund a customer account, "because cash paid in raises the bank's reserve at the central bank in the same unit of work, and there is no reserve to raise until a settlement agent has opened one", with `POST /deposits` answering `422` naming the membership.

That was an accurate description of the code and a false statement about banking. **A bank's counter has nothing to do with its central bank account.** A bank that has founded itself and joined no scheme can open its doors and take deposits; what it does with the money is hold it, as [vault cash](#vault-cash-and-the-lodgement) — its own money, in its own hands, in its own book. `POST /deposits` answers `200`.

What a founded bank cannot do is turn that cash into reserves, and that is where the refusal went. A **lodgement** is a request to the central bank, because only the central bank can credit an account in the central bank's book, and a bank with no reserve account to lodge into is refused there with `422` — `POST /lodgements`, and `payment.ErrSettlementMemberNotFound`. The check did not disappear; it moved to the act it was always true about.

**And it cannot pay or be paid**, which is a refusal rather than a consequence — `payment.ErrBankNotAdmitted`, `422`, made of both parties at `Mesh.Submit` and again by the clearing house before it takes a payment into a cycle. Saying it needed saying because it was for a while only an intention: an [arranged overdraft](#overdraft) funds a customer with no deposit at all, so "it has no money to pay with" was never the guard it was taken for, and the directory resolves a founded bank's IBANs like anyone else's, so nothing stopped one being paid either. What that cost when nothing refused it is the point: at the cut-off the clearing house cannot name a non-member in the settlement instruction, so the **whole cycle** stops with every other member's payments in it. A refusal at the door costs one payer a `422`; the absence of it stopped the network.
...
**Where the refusal lives, and what it cost when it did not exist.** Routing in this system is the mesh's actor table, not the clearing house's roster: a founded bank gets an actor the moment `Mesh.Admit` reserves its address, so nothing about the transport makes it unreachable. For two tasks nothing else did either. A payment addressed to a founded bank was submitted (`202`), relayed, accepted and cleared like any other; so was one **from** a founded bank, whose customer had spendable money from an arranged overdraft. Both failed at the *cut-off*, because the clearing house cannot name a non-member in the `pacs.009` it sends the central bank — and that takes the **whole cycle** down, not just the one payment: every other member's payments stay `Cleared`, their payees unpaid and their payers' money sitting in suspense. No remedy avoided admitting the bank: [`POST /cycles/{id}/settle`](#the-clearing-house--8082) failed in exactly the same place and rejecting the offending payment was refused `invalid payment state transition`.

It is refused now, of **both** parties and in two places: `Mesh.Submit`, the one door every submission comes through, so the refusal arrives before any leg is posted and as a `422`; and `AcceptAtCSMTx`, where the clearing house makes the same judgement from its own routing row before it takes a payment into a cycle. Both, and not one, because the clearing house's answer travels as a `pacs.002` addressed *through the roster* — so in the direction where the non-member is the payer, the rejection that would reverse its debtor leg has nowhere to go, and the money would stay in suspense. The one thing that made a founded bank genuinely unreachable before any of this was losing its actor, which a restart does (actors are rebuilt from the roster), and that is a property of the transport rather than a refusal anybody makes.
...
It cut the other way too, and that direction is easier to miss. `store/mem` was a map of Go strings and held any byte sequence at all; Postgres would not hold a NUL in a `text` column (SQLSTATE 22021) or in a `jsonb` string (22P05), nor anything that is not valid UTF-8. So `POST /members` with `{"name":"Ban\u0000k"}` — legal JSON — created a bank on one backend and returned a 500 carrying a raw SQLSTATE on the other. The fix went into neither store; it is `ledger.ValidateText`, one domain rule applied to every caller-supplied string that reaches a store, whether as a value it stores or as a key it looks a row up by:
...
Until Task 18a this table was on **no surface at all** and the route in its place resolved an IBAN across the whole network by sweeping every member's register. That was a clearing house reading every member's *deposit accounts*, which is not a thing this institution holds and, from Task 18c, not a thing it could reach — the `csm` shape has no deposit register at all. So the sweep left this port, and what replaced it is the answer a clearing house has always genuinely had. Address resolution as such now lives on a **bank's** port and answers out of that bank's own register.
...
**`GET /directory` is on one listener now, and it used to be on two.** The sentence that justified the second copy — *a bank is a scheme participant with genuine directory access* — is exactly what Task 18a retired. What the route does here is narrower and is a real question: which of **this bank's** accounts holds this address. A customer's own IBAN, an on-us payee, an operator checking an address before issuing another one. A payee at another bank is not something this bank can confirm, and the not-found it now gives is the honest answer rather than a regression.
