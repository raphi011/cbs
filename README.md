...
**A fresh deployment holds a BASE STATE, and a scenario is what fills it.** Boot leaves four banks founded, admitted at the settlement agent, on the clearing house's roster, subscribed to the routing directory, priced, capitalised and placed on reserve — and nothing else. There is no customer, no payment and no facility until somebody triggers one of `GET /scenarios`. Five ship: a payment carried the whole way, a payment handed over after the window shut, a settled payment returned, a borrower who falls behind, and the *demo network* that fills all four banks at once.
