...
A **bank has no cycles**. A member never *reads* the cycle row — `bankOps`, the narrowed interface a bank's handler holds, has no method that returns one. (It handles a cycle *id*: `PostSettlementAdvice` takes an `AdvisedMovement` carrying the `CycleID` the statement quoted, which is a reference to a row in somebody else's store.) A member learns of a cut-off from the `camt.053` addressed to it and from one `pacs.002` per payment, and its own record of that cut-off is the `settlement_advices` row keyed by `(book_id, cycle_id, asset)` — the first payment-layer table **keyed by** book. There is deliberately no foreign key from it to `cycles`: the constraint would encode exactly the sharing that splitting the stores removes.
...
What the scope may **not** span is more than one institution, and settlement is where that shows. It used to be one `Store.Update` holding the central bank's reserves and every member's creditor leg at once; it is now three units of work in three books, joined by messages, with each institution committing its own. See [SEPA Credit Transfer](#posting-choreography-sepa-credit-transfer) above.
