...
- **Returned**: after settlement, a SEPA R-transaction unwinds the flow. It is sent by the bank that *received* the original instruction — the payee's bank on a push, the payer's on a pull. The **reserve** half is the central bank's, because unwinding a settled payment moves reserves back and no member bank moves those; each **customer** leg is posted by the bank whose customer it moves, and the status becomes `Returned` when the second of the two lands.
