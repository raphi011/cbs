...
`Network.ResolveIdentifier` sweeps every member bank and refuses ambiguity rather than taking the first hit — the same rule settlement applies when it refuses to default a cycle's asset. That refusal lives in `deposit.Register` alone, deliberately with no `UNIQUE` constraint behind it: a constraint one store could hold and the other, a Go map, could not would let `store/mem` and `store/pg` disagree.
