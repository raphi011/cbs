...
func (s *surface) registerClockRoutes(mux *api.Router) {
	...
	// A door per phase, so a reader can run the clearing and stop. The day's own
	// declaration is the list of them and nothing here composes a sequence.
	mux.HandleFunc("GET /clock/phases", api.Handle(http.StatusOK, s.handleListPhases))
	mux.HandleFunc("POST /clock/phases/{phase}", api.Handle(http.StatusOK, s.handleRunPhase))
}
...
// handleRunPhase runs one phase and answers with what it moved. The clock stays
// where it stood, which is the whole difference between stepping a day and
// advancing one.
func (s *surface) handleRunPhase(r *http.Request) (api.PhaseReportDTO, error) {
	ctx, cancel := context.WithTimeout(context.WithoutCancel(r.Context()), dayTimeout)
	defer cancel()

	report, err := s.op.RunPhase(ctx, r.PathValue("phase"))
	if err != nil {
	...
	s.inst.Log().Info("business day phase",
		"phase", report.Phase.Key, "ran", report.Ran.Date,
		"files", len(report.Files), "outcomes", len(report.Outcomes), "problems", len(report.Problems))
	return report, nil
}
